...
All notable changes to the Botswana privileged admin backend are recorded here.
...
- **The site-selector dropdown/header only showed a country once it had a live row in
  `qos_reports`** — since Botswana had none yet, the dropdown rendered blank instead of showing
  "Botswana (bw)". `populateSiteSelect()` now always seeds every known country from `SITE_LABELS`
...
- **Ported the full Drill-down Explorer from `zwispqosp` v0.5.0/v0.5.1** (this app is kept as a
  ...
  analytics (City → Area → ISP → reports, now including Botswana's real, sourced areas from
  `bwispqosd` v1.2.0), ISP licensing, Benchmark reports, Moderation and Translations.
...
### Fixed
- Same rating-form truncation fix as `bwispqosd` v1.2.0 (CSS grid `0fr → 1fr` expand, no fixed
...
- Ported from zwispqosp v0.3.3: the Translations panel's "still-missing languages" note was
  hardcoded to Zimbabwe's list and was wrong here (Botswana's actual blank languages, per
  bwispqosd v1.1.0, are Kgalagadi, Mbukushu, Tshwa and !Xóõ — not the Zimbabwe list this panel was
  showing). Now renders the correct list for whichever site is selected via a `MISSING_LANGS` map,
  checked against each demo's real `I18N` content.
...
### Added
- First build for Botswana: the same admin app as zwispqosp (Supabase Auth + RLS,
  Google sign-in, passkeys, provider analytics, ISP licensing CRM, moderation,
  translation review, all six formatted exports), defaulting to `currentSite = "bw"`.
- Connects to the same shared Supabase project as zwispqosp/saispqosp (multi-tenant
...
- `PROVIDER_DIRECTORY.bw` populated (this was also added to zwispqosp itself in the
